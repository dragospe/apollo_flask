...
def request_user_access():
    ...
    if (request.method == 'POST'):
        ...
        with current_app.session_scope() as session:        
            ...
            if db_sid is not None:
                return render_template('oauth/garmin/consent.html', sid_already_registered = sid, project_name = current_app.config['PROJECT_NAME'])
    ...
    return render_template('oauth/garmin/consent.html', project_name = current_app.config['PROJECT_NAME'])
    ...
    with current_app.session_scope() as session:
        ...
        if db_sid is not None and db_sid.garmin_uid != uid.user_id:
            return Response(status = 409, response = render_template('oauth/garmin/callback.html',
                     sid_already_associated=rt.sid, project_name = current_app.config['PROJECT_NAME']))
        #Check if the garmin account in question is already associated with a different SID
            ...
            return Response(status = 409, response = render_template('oauth/garmin/callback.html',
                    uid_already_associated_with_sid = db_subject_uid, project_name = current_app.config['PROJECT_NAME']))
