...
Each of the "recieve_xyz" functions follows the same pattern: pick out the
approriate json, construct an object, (if necessary) upset time-offset samples,
...
def recieve_dailies():
    """
    Recieve daily summaries from Garmin.

    Loops trhough the json response, constructs "daily_summary" objects, and
    loads them into the database. Also upserts heart-rate sample data.
    """
        ...
        for summary in dailies:
            daily = daily_summary.\
                    Daily_Summary(
                        ...
                        start_time_local=datetime.fromtimestamp(\
                            summary.get('startTimeInSeconds')
                        ...
                        stress_duration=to_interval(
                            summary.get('stressDurationInSeconds')),
                        rest_stress_duration=to_interval(
                            summary.get('restStressDurationInSeconds')),
                        ...
                        intensity_duration_goal=to_interval(
                            summary.get('intensityDurationGoal')),
            ...
            if summary.get('timeOffsetHeartRateSamples') != {}:
                upsert_time_value_map(
                    ...
                    summary.get('timeOffsetHeartRateSamples'),
                    daily.start_time_local,
                    daily.sid,
    ...
    with current_app.session_scope() as session:
        for summary in activities:
            activity_summary = activity.Activity_Summary(
                ...
                avg_bike_cadence_rounds_per_minute=\
                    summary.get('averageBikeCadenceInRoundsPerMinute'),
                max_bike_cadence_rounds_per_minute=\
                    summary.get('maxBikeCadenceInRoundsPerMinute'),
...
def get_calendar_date(summary):
    calendar_date =\
        dateutil.parser.parse(
            summary.get('calendarDate')).date() \
            if summary.get('calendarDate') is not None else None
...
def update_db_from_api_response(db_session,
                                incoming_data):
    """[Parameters:]
    ...
    db_data = db_session.query(class_obj).filter_by(**pk_dict).one_or_none()

    if db_data is not None:
        db_session.delete(db_data)
        db_session.commit()

    db_session.add(incoming_data)


def uid2sid(db_session, uid):
    """Convert user identifier to subject identifier using an existing
    session."""
    subject = db_session.query(Subject).filter_by(garmin_uid=uid).one_or_none()
    if subject is None:
        return None
